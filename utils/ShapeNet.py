...
from utils.train_val_split import TRAIN_DICT, VAL_DICT, CLASS_NAME_TO_ID
import numpy as np
import configs.settings as settings
...
class ShapeNetDataset(Dataset):
    def __init__(self, cls_list=[], phase="train"):
        super().__init__()
        ...
        settings.init_indices(self.labels.unique().shape[0])
        ...
        if(self.phase == "train"):
            for key in TRAIN_DICT.keys():
                for model in TRAIN_DICT[key]:
                    labels.append(CLASS_NAME_TO_ID[key])
                    data.append(model)
